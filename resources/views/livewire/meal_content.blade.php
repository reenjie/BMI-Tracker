...
<select name="schedule" required class="form-select" id="">
    @php
        $bf = DB::select('select * from mealplans where dayid='.$d->id.' and schedule = 1 and rangeid = '.$rangeid.' ');
        $lu = DB::select('select * from mealplans where dayid='.$d->id.' and schedule = 2  and rangeid = '.$rangeid.' ');
        $di = DB::select('select * from mealplans where dayid='.$d->id.' and schedule = 3 and rangeid = '.$rangeid.'  ');
    @endphp
    @if(count($bf) <= 0)
...
<h6>Contents </h6>
<textarea name="content" required class="form-control txtcontent" id="" cols="30" placeholder="State all Recommended MealPlans" rows="10"></textarea>
<input type="hidden" name="dayid" value="{{$d->id}}">
</div>
<div class="modal-footer">
<button type="button" class="btn btn-secondary" data-bs-dismiss="modal" id="cloasemodal">Close</button>
...
$mealplans = DB::select('SELECT * FROM `mealplans` where dayid = '.$d->id.' and rangeid = '.$rangeid.'  order by schedule asc ');
@endphp
<div class="row">
